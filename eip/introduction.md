...
购买弹性IP时，可选择设置“出口”带宽上限，按照不同地域的单价，根据IP产生“出流量”每日零点进行计费。选择该计费方式时公网IP地址（裸IP）收费，不计带宽费用。
...
选择该计费方式时，费用分部分进行收取：保底带宽费用+后付费费用。EIP的实际带宽限制为峰值带宽，峰值带宽:保底带宽比例关系为5:1。

保底带宽费用在购买后付费带宽时收取，价格与普通带宽一致[价格列表](/network/unet/eip_price/accuratebandwidth)。

后付费费用会在次日零点结算，按超过保底部分带宽的平均值收费，即下图中阴影部分点的平均值（每五分钟取一个点，该点带宽值低于保底计为0，高于保底则按带宽值-保底计）。
![image](/images/eippostpaid.png)


举例：对保底5M，峰值25M的带宽，使用了25分钟，产生了五个计费带宽值分别为4,5,15,15,15。
...
使用共享带宽时，香港回内地带宽的监控信息由于采样方式和标准带宽计算有所不同，可能会有一定的偏差。此外香港回内地的带宽限制，由于算法问题，可能会短时超过限制值。关于香港回内地带宽的具体监控，可以在EIP界面看到，可以据此进行判断，香港回内地带宽是否超过限制。
