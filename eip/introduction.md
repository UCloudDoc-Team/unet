...
购买弹性IP时，可选择设置“出口”带宽上限，按照不同地域的单价，根据IP产生“出流量”每日零点后进行计费。选择该计费方式时公网IP地址（裸IP）收费，不计带宽费用。
...
使用共享带宽时，香港回内地带宽的监控信息由于采样方式和标准带宽计算有所不同，可能会有一定的偏差，此外香港回内地的带宽限制，由于算法问题，可能会短时超过限制值。

关于香港回内地带宽的具体监控，可以在EIP界面看到，可以据此进行判断，香港回内地带宽是否超过限制。
